...
@WebServlet("/profilImage")
public class ProfilImageServlet extends HttpServlet {
    public void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException {
        try {
// Lecture du BLOB
            Connection con = (Connection) request.getServletContext().getAttribute("DBConnection");
            PreparedStatement ps = con.prepareStatement("SELECT pp FROM tst.utilisateurs WHERE idutilisateur = ?");
            ps.setString(1, request.getParameter("id"));
            try (ResultSet rs = ps.executeQuery()) {
                rs.next();
                Blob blob = rs.getBlob("pp");
                if (blob != null) {
                    rs.close();
                    ps.close();
